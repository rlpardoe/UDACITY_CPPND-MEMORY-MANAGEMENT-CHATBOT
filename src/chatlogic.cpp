...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        {
                            _nodes.emplace_back(std::make_unique<GraphNode>(id)); //make_unique rather than new
                            newNode = _nodes.end() - 1; // get iterator to last element

                            // add all answers to current node
                            AddAllTokensToElement("ANSWER", tokens, **newNode); // dereference a std_unique is overloaded to return underlying object
                        }
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            (*childNode)->AddEdgeToParentNode(edge.get()); // give child raw ptr as handle
                            (*parentNode)->AddEdgeToChildNode(std::move(edge)); // unique ptr moved to parent
                        }
    ...
    //initialize ChatBot
    ChatBot chatBotManager = ChatBot("../images/chatbot.png");
    _chatBot = &chatBotManager;
    _chatBot->SetChatLogicHandle(this);

    // add chatbot to graph root node
    _chatBot->SetRootNode(rootNode);
    rootNode->MoveChatbotHere(std::move(chatBotManager));
